...
export async function GET(request: Request, ctx: Contexto) {
  ...
  const ponto = db.prepare("SELECT * FROM pontos WHERE id = ?").get(id);
  if (!ponto) {
...
export async function PUT(request: Request, ctx: Contexto) {
  ...
  const atualizado = db.prepare("SELECT * FROM pontos WHERE id = ?").get(id);
  return NextResponse.json(atualizado);
}

export async function DELETE(request: Request, ctx: Contexto) {
  const usuario = usuarioDoToken(request);
  if (!usuario) {
    return NextResponse.json({ erro: "Não autenticado" }, { status: 401 });
  }
  ...
  const resultado = db.prepare("DELETE FROM pontos WHERE id = ?").run(id);
